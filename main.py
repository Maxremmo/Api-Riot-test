...
class Game():
    ...
    def get_takedowns(self):
        ...
        takedowns = self.match['info']['participants'][self.partindex]['challenges']['takedowns']
    ...
    def get_multikills(self):
        ...
        multikills = self.match['info']['participants'][self.partindex]['challenges']['multikills']
...
def stats_to_df(username):
    ...
    the_df = my_df.copy()
    final_df = the_df[['assists', 'deaths', 'kills', 'champLevel', 'win', 'individualPosition', 'largestKillingSpree',
                'challenges.hadAfkTeammate', 'challenges.goldPerMinute', 'championName',
                'timePlayed', 'firstBloodKill']]
    final_df['timePlayed'] = round((final_df['timePlayed'] / 60),2)
    final_df['challenges.hadAfkTeammate'] = final_df['challenges.hadAfkTeammate'].fillna(False)
    final_df['challenges.hadAfkTeammate'] = final_df['challenges.hadAfkTeammate'].replace(1.0 , True)
    final_df.set_index('championName', inplace=True)
    very_final_df = final_df[['individualPosition','kills', 'deaths', 'assists','win','firstBloodKill',
                              'timePlayed', 'largestKillingSpree']]
    print(very_final_df)
